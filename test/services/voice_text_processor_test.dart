...
void main() {
  group('VoiceTextProcessor.clean — nettoyage générique (Partie 3)', () {
    ...
    test('supprime les mots de remplissage isolés (euh, hum, bah)', () {
      expect(
          VoiceTextProcessor.clean('euh je veux hum une pharmacie bah ouverte'),
  ...
  group(
      'VoiceTextProcessor.correctBusinessTerms — corrections exactes (Partie 5)',
      () {
    ...
    test('"ekbin" → "E-Kbine" (exemple explicite du prompt)', () {
      final r =
          VoiceTextProcessor.correctBusinessTerms('je veux commander un ekbin');
    ...
    test('"aben gourou" → "Abengourou"', () {
      expect(
          VoiceTextProcessor.correctBusinessTerms('je suis à aben gourou').text,
          contains('Abengourou'));
    });
    ...
    test('"ouave" → "Wave", "oranje money" → "Orange Money"', () {
      expect(VoiceTextProcessor.correctBusinessTerms('paye avec ouave').text,
          contains('Wave'));
      expect(
          VoiceTextProcessor.correctBusinessTerms('paye avec oranje money')
              .text,
  ...
  group(
      'VoiceTextProcessor.correctBusinessTerms — corrections floues (Partie 9)',
      () {
    test(
        'une variante proche mais jamais listée déclenche une correction FLOUE, pas silencieuse',
        () {
    ...
    test(
        'un terme déjà exactement correct ne déclenche aucune correction floue',
        () {
    ...
    test(
        'les mots courts (<4 caractères) ne déclenchent jamais de correction floue (trop de faux positifs)',
        () {
  ...
  group(
      'VoiceTextProcessor.process — pipeline complet, phrases réalistes par module (Partie 12)',
      () {
    ...
    for (final entry in cases.entries) {
      for (final phrase in entry.value) {
        test(
            '${entry.key} — "$phrase" ne plante jamais et retourne un texte non vide',
            () {
    ...
    test(
        'la pile complète (clean + correction) transforme un cas combiné réaliste',
        () {
      final r = VoiceTextProcessor.process(
          'euh je je veux euh un ekbin pour aller à café tout');
      expect(r.text, contains('E-Kbine'));
      expect(r.text, contains('Cafétou'));
