...
class AdminDashboard extends StatefulWidget {
  ...
  final bool twoFactorBypassed;
  const AdminDashboard({
    super.key,
    required this.adminData,
    this.twoFactorBypassed = false,
  });
  ...
  void initState() {
    ...
    if (widget.twoFactorBypassed) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        _twoFactorWarningOverlay = OverlayEntry(
          builder: (_) => const Positioned(
            left: 0,
            right: 0,
            bottom: 0,
            child: Material(child: AdminTwoFactorBypassBanner()),
          ),
        );
        Overlay.of(context, rootOverlay: true)
            .insert(_twoFactorWarningOverlay!);
      });
    }
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            // Avatar — photo de profil admin (Master Prompt
                            // "photos manquantes"), repli sur l'icône rôle
                            // si aucune photo n'est encore définie.
                            SinglePhotoEditor(
                              photoUrl: _photoUrl,
                              storagePath:
  ...
  List<Widget> _buildCards(BuildContext context) {
    ...
    return [
      ...
      if (_has('eventiel'))
        go(
            'Événementiel',
            'Prestataires & réservations',
            Icons.celebration_rounded,
            const [Color(0xFF6A1B9A), Color(0xFFE65100)],
      ...
      if (_has('sos'))
        go('Alertes SOS', 'Urgences livreurs', Icons.sos_rounded,
            const [Color(0xFFB71C1C), Color(0xFFE53935)], const AdminSosPage()),
      if (_has('anti_fraude'))
        go('Anti-fraude', 'Clients bloqués COD', Icons.gpp_bad_rounded,
            const [Color(0xFFBF360C), AppColors.primary], const AdminCodPage()),
      if (_has('cash_marchand'))
        go(
            'Cash à régler',
            'Espèces dues aux marchands',
      ...
      if (_has('ai_dashboard'))
        go(
            'Tableau de bord IA',
            'Conversations, coût, cache, outils',
...
@visibleForTesting
class AdminTwoFactorBypassBanner extends StatelessWidget {
  const AdminTwoFactorBypassBanner({super.key});

  @override
  Widget build(BuildContext context) => SafeArea(
        top: false,
        child: Container(
          key: const Key('admin-2fa-debug-bypass-banner'),
    ...
    if (_loading) {
      return const Row(
        children: [
          Expanded(
              child: AzShimmer(
                  width: double.infinity, height: 80, radius: AppRadius.xl)),
          SizedBox(width: 10),
          Expanded(
              child: AzShimmer(
                  width: double.infinity, height: 80, radius: AppRadius.xl)),
