      ...
      appBar: AppBar(
        ...
        actions: [
          TextButton.icon(
            ...
            label: Text('Déconnexion', style: GoogleFonts.inter(
        ...
        bottom: TabBar(
          ...
          tabs: const [
            Tab(icon: Icon(Icons.delivery_dining_rounded, size: 18), text: 'Livreurs'),
            Tab(icon: Icon(Icons.storefront_rounded, size: 18), text: 'Commerçants'),
            Tab(icon: Icon(Icons.mail_rounded, size: 18), text: 'Messages'),
            Tab(icon: Icon(Icons.sos_rounded, size: 18), text: 'SOS'),
            Tab(icon: Icon(Icons.lock_reset_rounded, size: 18), text: 'Réinitialisation'),
          ],
        ),
      ),
      body: Column(
        children: [
          ...
          Expanded(
            child: TabBarView(
              controller: _tabs,
              children: const [
                _ApplicationsList(collection: 'driver_applications', type: 'driver'),
                _ApplicationsList(collection: 'partner_applications', type: 'partner'),
                _ApplicationsList(collection: 'contact_messages', type: 'message'),
                _SosList(),
                _PasswordResetList(),
              ],
            ),
    ...
    return Container(
      ...
      child: const Row(
        children: [
          ...
          SizedBox(width: 12),
          _StatChip('sos_alerts',           'SOS actifs',  Colors.red,
            filter: 'active'),
        ],
      ),
    );
...
class _StatChip extends StatelessWidget {
  ...
  final Color color;
  final String filter;
  const _StatChip(this.collection, this.label, this.color, {this.filter = 'unread'});
      ...
      builder: (_, snap) {
        ...
        return Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(color: count > 0 ? color.withValues(alpha: 0.3) : kDivider),
          ),
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            ...
            Text(label, style: GoogleFonts.inter(
              fontSize: 13, color: count > 0 ? color : kTextMuted, fontWeight: FontWeight.w600)),
          ]),
        );
      },
    );
        ...
        if (!snap.hasData || snap.data!.docs.isEmpty) {
          return Center(
            child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(Icons.inbox_rounded, size: 56, color: kTextMuted.withValues(alpha: 0.4)),
...
class _EntryCard extends StatelessWidget {
  ...
  final String collection;
  final String type;
  const _EntryCard({required this.doc, required this.collection, required this.type});
  ...
  Future<void> _delete(BuildContext context) async {
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        title: Text('Confirmer', style: GoogleFonts.inter(fontWeight: FontWeight.w700)),
  ...
  Widget build(BuildContext context) {
    ...
    switch (status) {
      case 'treated':
        statusColor = kSuccess; statusLabel = 'Traité'; break;
      case 'in_progress':
        statusColor = kBlue; statusLabel = 'En cours'; break;
      default:
        statusColor = kOrange; statusLabel = 'Nouveau'; break;
    }
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: status == 'unread' ? kOrange.withValues(alpha: 0.3) : kDivider),
      ...
      child: Theme(
        ...
        child: ExpansionTile(
          ...
          subtitle: Text(date, style: GoogleFonts.inter(
  ...
  IconData _typeIcon() {
    switch (type) {
      ...
      case 'partner': return Icons.storefront_rounded;
  ...
  Widget _detailRows(Map<String, dynamic> data) {
    ...
    void add(String k, String? v) { if (v != null && v.isNotEmpty) rows.add((k, v)); }
    ...
    return Column(
      ...
      children: rows.map((r) => Padding(
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          SizedBox(
            width: 100,
            child: Text(r.$1, style: GoogleFonts.inter(
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (_, snap) {
        ...
        if (!snap.hasData || snap.data!.docs.isEmpty) {
          return Center(
            child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(Icons.lock_open_rounded, size: 56, color: kTextMuted.withValues(alpha: 0.4)),
              const SizedBox(height: 12),
            ...
            Container(
              ...
              child: Row(children: [
                ...
                Expanded(child: Text(
                  ...
                  style: GoogleFonts.inter(fontSize: 12, color: kTextMuted, height: 1.5),
                )),
              ]),
            ),
            Expanded(
                  ...
                  return Container(
                    ...
                    child: Row(children: [
                      ...
                      Expanded(child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(data['name'] ?? 'Client', style: GoogleFonts.inter(
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (_, snap) {
        ...
        if (!snap.hasData || snap.data!.docs.isEmpty) {
          return Center(
            child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(Icons.sos_rounded, size: 56, color: kTextMuted.withValues(alpha: 0.4)),
              const SizedBox(height: 12),
...
class _SosCard extends StatelessWidget {
  ...
  Future<void> _delete(BuildContext context) async {
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        title: Text('Confirmer', style: GoogleFonts.inter(fontWeight: FontWeight.w700)),
        content: Text('Supprimer cette alerte SOS ?', style: GoogleFonts.inter()),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, false),
            child: const Text('Annuler')),
          TextButton(onPressed: () => Navigator.pop(context, true),
    ...
    if (ok == true) {
      await FirebaseFirestore.instance.collection('sos_alerts').doc(doc.id).delete();
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final name   = data['clientName'] as String? ?? data['driverName'] as String? ?? 'Inconnu';
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: kCard,
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: cardColor.withValues(alpha: isActive ? 0.4 : 0.2)),
          ...
          Row(children: [
            ...
            Expanded(child: Column(
              ...
              children: [
                ...
                Text(
                  '${type == 'driver' ? 'Livreur' : 'Client'}  •  $date',
                  style: GoogleFonts.inter(fontSize: 12, color: kTextMuted)),
              ],
            )),
            Container(
              ...
              child: Text(
                ...
                style: GoogleFonts.inter(
                  fontSize: 11, color: cardColor, fontWeight: FontWeight.w800,
                  letterSpacing: 0.5),
              ),
            ),
          ]),
              ...
              child: Container(
                ...
                child: Row(children: [
                  ...
                  Text(
                    ...
                    style: GoogleFonts.inter(
                      fontSize: 12, color: const Color(0xFF4285F4),
